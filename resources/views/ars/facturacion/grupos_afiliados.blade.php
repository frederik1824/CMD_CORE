@extends('layouts.ars')
@section('title', 'Facturación Corporativa')
@section('content')
<div class="space-y-6 font-sans text-xs">
    <div class="flex items-center justify-between pb-4 border-b border-slate-100">
        <div>
            <h2 class="text-xl font-bold text-slate-900 tracking-tight">Facturación Corporativa / Grupos</h2>
            <p class="text-xs text-slate-500 font-medium">Facturación colectiva enviada a patronos y grupos asociados.</p>
        </div>
    </div>

    <div class="bg-white rounded-3xl border border-slate-100 p-6 shadow-xs space-y-4 animate-fade-in">
        <h3 class="font-bold text-slate-800">Facturación Colectiva</h3>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-100">
                <thead class="bg-slate-50 font-bold text-slate-400">
                    <tr>
                        <th class="px-4 py-3 text-left">No. Factura</th>
                        <th class="px-4 py-3 text-left">Grupo Corporativo</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white font-medium">
                    @forelse($invoices as $inv)
                        <tr>
                            ...
                            <td class="px-4 py-3 font-semibold text-slate-700">{{ $inv->group->name }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-6 text-center text-slate-400 font-semibold">No se registran facturas corporativas en base de datos.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
