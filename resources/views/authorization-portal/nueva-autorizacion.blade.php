...
<div class="max-w-6xl mx-auto space-y-6" x-data="autorizacionPortal">
        ...
        <div class="bg-white text-slate-800 p-8 rounded-3xl border border-slate-100 shadow-2xl max-w-sm w-full text-center space-y-6 animate-fade-in-up">
            ...
            <div class="space-y-2">
                ...
                <p class="text-xs text-slate-400 leading-relaxed">
                    Estableciendo conexión segura con los servidores de **ARS CMD**...
                </p>
            </div>
            ...
            <div class="text-[10px] bg-slate-50 border border-slate-100 rounded-2xl p-4 text-left font-mono space-y-1.5 text-slate-500">
                <div class="flex items-center space-x-2">
                    ...
                    <span :class="submitStep >= 1 ? 'text-emerald-700 font-bold' : 'text-slate-700 font-bold animate-pulse'">1. Transmitiendo datos de afiliado...</span>
                </div>
                <div class="flex items-center space-x-2">
                    ...
                    <span :class="submitStep >= 2 ? 'text-emerald-700 font-bold' : (submitStep === 1 ? 'text-slate-700 font-bold animate-pulse' : '')">2. Verificando coberturas...</span>
                </div>
                <div class="flex items-center space-x-2">
                    ...
                    <span :class="submitStep >= 3 ? 'text-emerald-700 font-bold' : (submitStep === 2 ? 'text-slate-700 font-bold animate-pulse' : '')">3. Generando Constancia...</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center space-x-4 pb-4 border-b border-slate-200">
        ...
        <div class="flex items-center space-x-3.5">
            ...
            <div class="border-l border-slate-300 pl-3.5 leading-none py-1">
                <span class="text-[#000666] font-extrabold text-base tracking-wider block font-title">ARS CMD</span>
                <span class="text-[#0056c5] font-bold text-[9px] tracking-wider block mt-0.5 uppercase">Seguro Familiar de Salud (SFS)</span>
            </div>
        </div>
        <div class="border-l border-slate-200 pl-4">
            <h2 class="text-base font-extrabold text-slate-800">Portal de Autorizaciones</h2>
            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Ingreso de Solicitudes Médicas</p>
        </div>
    </div>

    <!-- Mensajes de Error de Validación -->
    <div x-show="errorMsg" x-transition class="p-4 bg-rose-50 border border-rose-200 rounded-2xl flex items-start space-x-3 shadow-md text-xs text-rose-800" x-cloak>
    ...
    <form action="{{ route('pss.autorizaciones.guardar') }}" method="POST" enctype="multipart/form-data" class="space-y-6" @submit.prevent="submitForm($event)">
        @csrf
        ...
        <div class="bg-white p-6 rounded-3xl border-t-4 border-t-[#49bcf7] border-x border-b border-slate-200 shadow-md space-y-5">
            <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center space-x-1.5 border-b border-slate-100 pb-2">
                <svg class="w-4 h-4 text-teal-650" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" x-model="searchQuery" @input="formatQuery"
                           class="block w-full rounded-full border border-slate-300 bg-[#eaf1fb]/15 pl-11 pr-4 py-2.5 text-xs text-slate-800 focus:bg-white focus:border-[#49bcf7] focus:ring-2 focus:ring-[#49bcf7]/15 transition-all font-semibold"
                ...
                <button type="button" @click="validarAfiliado" :disabled="loading"
                        class="px-6 py-2.5 rounded-full text-xs font-extrabold text-white bg-teal-600 hover:bg-teal-700 hover:shadow-md disabled:bg-slate-300 disabled:text-slate-400 flex items-center justify-center space-x-2 transition shadow-xs">
        ...
        <div x-show="afiliado" x-transition class="bg-white p-6 rounded-3xl border-t-4 border-t-[#0be881] border-x border-b border-slate-200 shadow-md space-y-5" x-cloak>
            <h3 class="text-xs font-bold text-[#0f766e] flex items-center space-x-1.5 border-b border-slate-100 pb-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs border border-slate-150 rounded-2xl p-4 bg-slate-50/30">
                <!-- Columna 1 -->
                <div class="space-y-4 border-r border-slate-200 pr-4 last:border-0">
                    <div>
                        <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Nombres</span>
                        <span class="font-extrabold text-slate-800 text-sm mt-0.5" x-text="afiliado ? afiliado.nombres : ''"></span>
                    </div>
                    <div>
                ...
                <div class="space-y-4 border-r border-slate-200 pr-4 last:border-0">
                    <div>
                        ...
                        <span class="font-extrabold text-slate-800 text-sm mt-0.5" x-text="afiliado ? afiliado.apellidos : ''"></span>
                    </div>
                    <div>
        ...
        <div class="bg-white p-6 rounded-3xl border-t-4 border-t-[#403663] border-x border-b border-slate-200 shadow-md space-y-5">
            <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center space-x-1.5 border-b border-slate-100 pb-2">
                <svg class="w-4 h-4 text-teal-650" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                ...
                <div>
                    <label for="tipo_servicio" class="block font-bold text-slate-450 mb-1.5 uppercase tracking-wider text-[9px]">Tipo de Autorización <span class="text-rose-500">*</span></label>
                    <select name="tipo_servicio" id="tipo_servicio" x-model="tipoServicio" :disabled="!afiliado" required
                            class="block w-full rounded-full border border-slate-300 bg-[#eaf1fb]/15 py-2.5 px-4 text-xs text-slate-800 focus:bg-white focus:border-[#49bcf7] focus:ring-2 focus:ring-[#49bcf7]/15 transition-all disabled:bg-slate-50 disabled:text-slate-400 font-semibold">
                        <option value="consulta font-semibold">Consulta Médica Programada</option>
                ...
                <!-- Diagnóstico Autocomplete Searchable -->
                <div class="relative" x-data="{ open: false }" @click.away="open = false">
                    <label class="block font-bold text-slate-450 mb-1.5 uppercase tracking-wider text-[9px]">Diagnóstico <span class="text-rose-500">*</span></label>
                    
                    <div class="relative">
                        <input type="text" 
                               x-model="diagnostico" 
                               @focus="open = true; if(!diagnostico) diagnostico = ''"
                               @input="open = true"
                               :disabled="!afiliado" required
                               class="block w-full rounded-full border border-slate-300 bg-[#eaf1fb]/15 py-2.5 pl-4 pr-10 text-xs text-slate-850 focus:bg-white focus:border-[#49bcf7] focus:ring-2 focus:ring-[#49bcf7]/15 transition-all disabled:bg-slate-50 disabled:text-slate-400 font-semibold"
                               placeholder="Buscar diagnóstico o código CIE-10">
                        
                        <button type="button" @click="if(afiliado) open = !open" :disabled="!afiliado"
                                class="absolute right-3 top-2.5 text-slate-400 hover:text-slate-600 disabled:opacity-50">
                            <svg class="h-4 w-4 transform transition-transform" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div x-show="open && searchedDiagnosticos.length > 0" 
                         x-transition
                         class="absolute z-50 mt-1 w-full max-h-60 overflow-y-auto rounded-2xl bg-white border border-slate-200 shadow-lg py-1 text-xs" x-cloak>
                        <template x-for="diag in searchedDiagnosticos" :key="diag">
                            <button type="button" 
                                    @click="
                                        ...
                                        open = false;
                                    " 
                                    class="w-full text-left px-4 py-2.5 hover:bg-slate-50 focus:bg-slate-50 text-slate-700 font-semibold transition border-b border-slate-50 last:border-b-0"
                                    x-text="diag">
                            </button>
                        </template>
                    </div>
                    ...
                    <div x-show="open && searchedDiagnosticos.length === 0" 
                         class="absolute z-50 mt-1 w-full rounded-2xl bg-white border border-slate-200 shadow-lg p-4 text-center text-slate-400 font-bold" x-cloak>
                ...
                <div>
                    <label class="block font-bold text-slate-450 mb-1.5 uppercase tracking-wider text-[9px]">Teléfono <span class="text-rose-500">*</span></label>
                    <input type="text" x-model="telefono" @input="formatPhone" maxlength="12" name="telefono"
                           :disabled="!afiliado" required
                           class="block w-full rounded-full border border-slate-300 bg-[#eaf1fb]/15 px-4 py-2.5 text-xs text-slate-800 focus:bg-white focus:border-[#49bcf7] focus:ring-2 focus:ring-[#49bcf7]/15 transition-all disabled:bg-slate-50 disabled:text-slate-400 font-mono font-bold"
                           placeholder="000-000-0000">
                </div>
            </div>
            
            <!-- Adjuntar receta -->
            <div class="border border-dashed border-slate-300 rounded-2xl p-4 bg-slate-50/30 flex flex-col sm:flex-row items-center justify-between space-y-2 sm:space-y-0 text-xs">
                <div>
                    <p class="font-bold text-slate-650 text-xs">Adjuntar Indicación Médica / Receta (Simulado)</p>
                ...
                <input type="file" name="documento_soporte" :disabled="!afiliado"
                       class="text-xs text-slate-500 file:mr-3 file:py-1.5 file:px-4 file:rounded-full file:border-0 file:text-[10px] file:font-bold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100 cursor-pointer disabled:opacity-50 font-bold">
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl border-t-4 border-t-[#49bcf7] border-x border-b border-slate-200 shadow-md space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs items-end">
                ...
                <div>
                    <label class="block font-bold text-slate-450 mb-1.5 uppercase tracking-wider text-[9px]">Grupo Médico</label>
                ...
                <!-- Dropdown Servicio Médico Searchable -->
                <div class="relative" x-data="{ open: false }" @click.away="open = false">
                    <label class="block font-bold text-slate-450 mb-1.5 uppercase tracking-wider text-[9px]">Servicio Médico</label>
                    
                    <div class="relative">
                        <input type="text" 
                               x-model="servicioSearchQuery" 
                               @focus="open = true; if(!servicioSearchQuery) servicioSearchQuery = ''"
                               @input="open = true"
                               :disabled="!afiliado || !grupoMedico"
                               class="block w-full rounded-full border border-slate-300 bg-[#eaf1fb]/15 py-2.5 pl-4 pr-10 text-xs text-slate-850 focus:bg-white focus:border-[#49bcf7] focus:ring-2 focus:ring-[#49bcf7]/15 transition-all disabled:bg-slate-50 disabled:text-slate-400 font-semibold"
                               placeholder="Escriba código o nombre...">
                        
                        <button type="button" @click="if(afiliado && grupoMedico) open = !open" :disabled="!afiliado || !grupoMedico"
                                class="absolute right-3 top-2.5 text-slate-400 hover:text-slate-600 disabled:opacity-50">
                            <svg class="h-4 w-4 transform transition-transform" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                            </svg>
                    ...
                    <!-- Dropdown Results -->
                    <div x-show="open && searchedServicios.length > 0" 
                         x-transition
                         class="absolute z-50 mt-1 w-full max-h-60 overflow-y-auto rounded-2xl bg-white border border-slate-200 shadow-lg py-1 text-xs" x-cloak>
                        <template x-for="srv in searchedServicios" :key="srv.id">
                            <button type="button" 
                                    @click="
                                        servicioId = srv.id; 
                                        servicioSearchQuery = srv.codigo + ' - ' + srv.descripcion;
                                        updateValorDefault();
                                        open = false;
                                    " 
                                    class="w-full text-left px-4 py-2 hover:bg-slate-50 focus:bg-slate-50 text-slate-700 font-semibold transition flex flex-col gap-0.5 border-b border-slate-50 last:border-b-0">
                                <span class="font-mono font-bold text-[#49bcf7]" x-text="srv.codigo"></span>
                                <span class="text-slate-800" x-text="srv.descripcion"></span>
                            </button>
                        </template>
                    </div>
                ...
                <div>
                    <button type="button" @click="agregarServicio" :disabled="!afiliado || !servicioId || incompatibilidadMsg !== ''"
                            class="w-full px-5 py-2.5 border border-transparent rounded-full shadow-xs text-xs font-bold text-white bg-[#49bcf7] hover:bg-[#31a3e6] hover:shadow-md disabled:bg-slate-350 disabled:text-slate-400 flex items-center justify-center space-x-1.5 transition">
        ...
        <div class="bg-white p-6 rounded-3xl border-t-4 border-t-[#403663] border-x border-b border-slate-200 shadow-md space-y-5 overflow-hidden">
            <div class="flex justify-between items-center border-b border-slate-100 pb-2">
                ...
                <!-- Badge Dinámico -->
                <span class="inline-flex items-center rounded-full bg-[#403663] px-3 py-1 text-[10px] font-bold text-white tracking-wider"
                      x-text="countServicios + ' servicios'"></span>
            </div>

            <!-- Tabla de Servicios -->
            <div class="border border-slate-100 rounded-xl overflow-hidden bg-slate-50/20">
                <table class="w-full table-fixed divide-y divide-slate-100 text-xs">
                    <thead class="bg-slate-50/50 font-bold text-slate-400">
                        <tr>
                            <th scope="col" class="px-4 py-3 text-left w-24 text-[9px] uppercase tracking-wider whitespace-nowrap" style="text-align: left;">Código</th>
                            <th scope="col" class="px-4 py-3 text-left text-[9px] uppercase tracking-wider" style="text-align: left;">Nombre</th>
                            <th scope="col" class="px-4 py-3 w-40 text-[9px] uppercase tracking-wider whitespace-nowrap" style="text-align: right;">Valor (Reclamado)</th>
                            <th scope="col" class="px-4 py-3 w-36 text-[9px] uppercase tracking-wider whitespace-nowrap" style="text-align: right;">Cobertura</th>
                            <th scope="col" class="px-4 py-3 w-36 text-[9px] uppercase tracking-wider whitespace-nowrap" style="text-align: right;">Diferencia</th>
                            <th scope="col" class="px-4 py-3 w-16 text-[9px] uppercase tracking-wider whitespace-nowrap" style="text-align: center;">Acción</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 bg-white">
                        ...
                        <template x-for="(item, index) in serviciosAgregados" :key="item.id">
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-4 py-2.5 font-mono font-bold text-slate-500 whitespace-nowrap" style="text-align: left;" x-text="item.codigo"></td>
                                <td class="px-4 py-2.5 text-slate-700 font-semibold" style="text-align: left;" x-text="item.descripcion"></td>
                                <td class="px-4 py-2.5 text-right font-bold text-slate-900" style="text-align: right;">
                                ...
                                <td class="px-4 py-2.5 text-right text-slate-500 font-semibold font-mono whitespace-nowrap" style="text-align: right;" x-text="'DOP ' + parseFloat(item.cobertura).toFixed(2)"></td>
                                <td class="px-4 py-2.5 text-right text-slate-700 font-bold font-mono whitespace-nowrap" style="text-align: right;" x-text="'DOP ' + parseFloat(item.diferencia).toFixed(2)"></td>
                                <td class="px-4 py-2.5 text-center whitespace-nowrap" style="text-align: center;">
                        ...
                        <template x-if="countServicios === 0">
                            <tr>
                                <td colspan="6" class="px-4 py-6 text-center text-slate-400 font-semibold whitespace-nowrap">
                        ...
                        <!-- Fila TOTAL GENERAL -->
                        <tr class="bg-slate-50/50 font-bold text-slate-805 border-t border-slate-100 whitespace-nowrap">
                            <td colspan="2" class="px-4 py-3 uppercase tracking-wider text-[10px] text-slate-500 font-bold whitespace-nowrap" style="text-align: left;">TOTAL GENERAL</td>
                            <td class="px-4 py-3 text-right text-slate-900 text-xs font-mono font-extrabold whitespace-nowrap" style="text-align: right;" x-text="'DOP ' + parseFloat(totalValor).toFixed(2)"></td>
                            <td class="px-4 py-3 text-right text-emerald-700 text-xs font-mono font-extrabold whitespace-nowrap" style="text-align: right;" x-text="'DOP ' + parseFloat(totalCobertura).toFixed(2)"></td>
                            <td class="px-4 py-3 text-right text-slate-900 text-xs font-mono font-extrabold whitespace-nowrap" style="text-align: right;" x-text="'DOP ' + parseFloat(totalDiferencia).toFixed(2)"></td>
                            <td class="px-4 py-3 whitespace-nowrap" style="text-align: center;"></td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="flex justify-end space-x-3 pt-4 border-t border-slate-50">
                ...
                <button type="submit" :disabled="!afiliado || countServicios === 0"
                        class="px-6 py-2.5 border border-transparent rounded-full shadow-sm text-xs font-extrabold text-white bg-teal-600 hover:bg-teal-700 hover:shadow-md disabled:bg-slate-300 disabled:text-slate-400 flex items-center space-x-2 transition active:scale-95">
                    ...
                    <span>Enviar Solicitud ARS</span>
                </button>
            </div>
        </div>
    </form>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('autorizacionPortal', () => ({
        ...
        init() {
            ...
            this.$watch('grupoMedico', value => {
                this.servicioId = '';
                this.valorServicio = '';
                this.servicioSearchQuery = '';
            });

            this.$watch('servicioSearchQuery', value => {
                if (!value) {
                    this.servicioId = '';
                    this.valorServicio = '';
                }
        ...
        get searchedServicios() {
            const query = (this.servicioSearchQuery || '').toLowerCase().trim();
            const list = this.filteredServicios;
            if (this.servicioId) {
                const selected = list.find(s => s.id == this.servicioId);
                if (selected && (selected.codigo + ' - ' + selected.descripcion) === this.servicioSearchQuery) {
                    return list;
                }
            }
