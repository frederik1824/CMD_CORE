                        ...
                        <div class="bg-white border border-slate-100 rounded-xl p-4 flex items-center justify-between shadow-xs hover:shadow-sm transition">
                            <div class="space-y-1">
                                ...
                                <span class="text-xs text-slate-600 font-medium block">
                                    {{ optional($newAut->servicio)->descripcion
                                        ?? optional($newAut->servicioPdss)->coverage_description
                                        ?? $newAut->procedimiento
                                        ?? '—' }}
                                </span>
                    ...
                    @forelse($solicitudes as $sol)
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="px-6 py-4.5 whitespace-nowrap text-slate-600 font-semibold">
                                {{ optional($sol->servicio)->descripcion
                                    ?? optional($sol->servicioPdss)->coverage_description
                                    ?? $sol->procedimiento
                                    ?? '—' }}
                            </td>
                            ...
                            <td class="px-6 py-4.5 text-slate-500 max-w-xs leading-normal font-medium" x-data="{ expanded: false }">
                                @php
                                    $motivo = $sol->motivo_estado ?? 'Solicitud en revisión en bandeja de autorizaciones.';
                                    $short = mb_strlen($motivo) > 45 ? mb_substr($motivo, 0, 45) . '...' : $motivo;
                                    $hasMore = mb_strlen($motivo) > 45;
                                @endphp
                                
                                @if($hasMore)
                                    <div class="flex items-start gap-1">
                                        <span x-show="!expanded" class="text-slate-500">{{ $short }}</span>
                                        <span x-show="expanded" class="text-slate-650 block whitespace-normal" x-cloak>{{ $motivo }}</span>
                                        <button type="button" @click="expanded = !expanded" class="text-[#49bcf7] hover:text-[#403663] font-bold text-xs select-none focus:outline-none flex-shrink-0 ml-1">
